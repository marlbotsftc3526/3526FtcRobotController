...
@Autonomous(name="LimelightSampleAuto", group="Linear OpMode")
@Config
public class LimelightSampleAuto extends LinearOpMode {
            ...
            switch (currentState) {
                case DRIVE_TO_SCORE:
                    ...
                    if ((robot.drivetrain.targetReached)|| timer.seconds() > 1.5) { //1.5
                        currentState = LimelightSampleAuto.State.SCORING;
                        timer.reset();
                    }
                    break;
                case SCORING:
                    ...
                    if(robot.lift.liftLeft.getCurrentPosition() > robot.lift.highbucketpos - 40){
                        ...
                        if(timer.seconds() > 2){ //1.5
                            ...
                            if(timer.seconds() > 3) {
                                if (counter<1) {
                                    currentState = LimelightSampleAuto.State.DRIVE_TO_INTAKE;
                                    ...
                                    currentState = LimelightSampleAuto.State.DRIVE_TO_SUB;
                                    timer.reset();
                ...
                case DRIVE_TO_INTAKE:
                    ...
                    if (robot.drivetrain.targetReached || timer.seconds() > 2.1) {
                        currentState = LimelightSampleAuto.State.INTAKING;
                        timer.reset();
                    }
                    break;
                case INTAKING:
                    if(timer.seconds() > 0.3) { // 0.3
                        ...
                        if (timer.seconds() > .5) { //.5
                            ...
                            robot.extension.extMode = Extension.ExtMode.FARBACK;
                            if (timer.seconds() > 1) { //1
                                currentState = LimelightSampleAuto.State.DRIVE_TO_SCORE;
                                robot.drivetrain.setTargetPose(scorePose);
                                counter++;
                                timer.reset();
                            }
                ...
                case DRIVE_TO_SUB:
                    ...
                    if (timer.seconds()>3) {
                        robot.aClaw.clawPivot.setPosition(robot.aClaw.pivotDOWN);
                        currentState = LimelightSampleAuto.State.ALIGN;
                        timer.reset();
                    }
                    break;
                ...
                case ALIGN:
                    ...
                    robot.lift.liftMode = Lift.LiftMode.GROUNDACTIVE;
                    currentState = LimelightSampleAuto.State.GET;
                    break;
                case GET:
                    while (timer.seconds()<5) {
                       // robot.drivetrain.limeLight();
                    ...
                    robot.aClaw.clawPivot.setPosition(robot.aClaw.pivotSCORE);
                    robot.extension.extMode = Extension.ExtMode.NEAR;
                    newSub = new Pose2D(DistanceUnit.INCH, parkX, parkY - 10, AngleUnit.DEGREES, parkT);
                    robot.drivetrain.setTargetPose(newSub);
                    timer.reset();
                            if (timer.seconds()>2) {
